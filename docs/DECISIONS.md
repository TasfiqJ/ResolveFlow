...
| U-071 | `pip-audit` and `pnpm audit` reported no known dependency vulnerabilities for the 2026-07-22 release. Pinned Gitleaks 8.30.1 scanned 18 reachable commits with no leaks after one exact-fingerprint ignore for the documented synthetic identifier `prior-incident-1042`. | Release verifier output on 2026-07-22 and `.gitleaksignore`; this is dated evidence, not a permanent claim. The later browser-tooling addition is covered by D-048. | New dependency/history findings require remediation or an explicit bounded exception. |
...
## 12. Post-release public UX decisions

| ID | Decision / fact | Rationale / evidence | Reversal trigger |
|---|---|---|---|
| D-044 | Use a story-first public homepage that explains the problem, the Resolve/Replay/Gate loop, one recorded synthetic walkthrough, deterministic controls, proof limits, and then optional technical detail. Keep the dense trace, scorecard, raw snapshots, and architecture on linked detail routes. | Desktop browser review showed the previous first viewport presented six technical navigation choices, provenance IDs, three release cards, and a dense audit workspace before a visitor could build a simple mental model. The revised path preserves the complete snapshot-first product while making its purpose and primary action visible on first load. | Observed user testing shows another information order produces better comprehension without weakening provenance or claim-boundary visibility. |
| D-045 | Present ResolveFlow as a performance-led engineering case study: builder attribution and one-sentence product definition first, a native animated Replay engine as the focal visual, then recorded control-room evidence, system architecture, exact repository proof, ownership scope, and deep links. Recreate the supplied reference's visual language in project-owned HTML/CSS instead of loading its third-party animation and suppressing attribution. | The story-first revision clarified the concept but still read like product documentation rather than a hiring portfolio. The supplied reference demonstrated the desired cinematic technical framing. Native motion keeps the snapshot-first experience fast and complete without CDN availability, licensing ambiguity, or hidden branding behavior. | Measured portfolio review shows a quieter presentation converts better, or the owned motion materially harms accessibility or page performance. |

## 13. Post-release credibility-hardening decisions

| ID | Decision / fact | Rationale / evidence | Reversal trigger |
|---|---|---|---|
| D-046 | Replace the inert Replay selector with a real accessible evidence explorer and distinguish `RECORDED` snapshot evidence from `AUTOMATED TEST` evidence. | A control that did not affect the page made the demo look staged. Only role downgrade and its baseline have public recorded snapshots; three additional scenarios have automated tests but no public run artifact. | New checksummed public snapshots are generated through the shared path and can be labeled recorded. |
| D-047 | Keep the HTML string smoke as a fast artifact check, add Playwright/Axe Chromium coverage, gate Pages on repository verification, and pin every GitHub Action to a full commit SHA. | Local Playwright found 36 repeated low-contrast labels and one mobile overflow defect that string parsing could not detect; nine browser checks pass after correction, including nested navigation under the Pages base path. Deployment should consume the same evidence boundary that the product advertises. | A stronger equivalent browser/accessibility suite or trusted workflow policy replaces these checks without reducing coverage. |
| D-048 | Fail dependency verification on high-severity production findings and critical development findings; document one high-severity development-only `brace-expansion` advisory in the ESLint 9 tree. | Production audit passes. A global override to `brace-expansion` 5 breaks `minimatch` 3, while ESLint 10 is outside the peer range of the current Next/React/import plugins. Hiding the result or claiming a clean full audit would be misleading. | Compatible plugin releases support ESLint 10 or the transitive dependency is patched without breaking lint, after which the exception is removed and the full high threshold is restored. |
| D-049 | Supersede draft gate 1.0 with fail-closed gate 1.1. A hard metric with `not_exercised` or `not_verified` evidence status is `insufficient_sample` and release-blocking rather than a passing zero. | The role-downgrade Replay never dispatched an action, reconciled a retry, or inspected a deployed public environment, yet gate 1.0 credited payload mismatch, duplicate action, and public credential metrics as `0/1`. | A compatible Replay or independently checksummed operational evidence actually exercises each boundary and is aggregated without changing the declared threshold after results. |
| D-050 | Treat the 36 truth IDs as one semantic draft template until distinctness and human review are demonstrated. | A canonical semantic fingerprint over timeline, domain, evidence inventory, answerability, route, unknowns, action, and mutation expectations produces 1 unique payload and one 36-ID duplicate group. Generating cosmetic variants would not solve evaluation independence. | Distinct cases with reviewed latent truths replace or version the draft catalog and pass semantic/leakage auditing. |
| D-051 | Treat the 200-cell security matrix as a declaration plan, not execution evidence. | The matrix expands to 200 identifiers but has 0 full Replay executions, 5 unique payloads for 20 family/variant slots, and previously had one family-name mismatch. The mismatch is fixed; execution coverage remains open. | Checksummed scenario manifests materialize each claimed cell, execute the declared suite, and retain per-case outcomes and failures. |
| D-052 | Publish and independently recompute the run audit hash chain. | `RunSnapshot.trace` previously serialized the base `TraceEvent` type, dropping hash fields from public bundles, while the scorer trusted stored references without recomputing hashes. The typed snapshot now retains `AuditEvent` fields and tamper regression coverage. | A cryptographically stronger signed transparency-log design supersedes the current deterministic hash chain. |
| D-053 | Compose live chat, embedding, and reranking adapters as one authorized runtime choice and derive snapshot provenance from the active provider. | The API previously selected Cohere Chat while the orchestrator silently retained fixture embeddings/reranking, ignored configured agent models/budgets, and structurally labeled every run `recorded_fixture`. Contract coverage now proves that non-fixture embeddings are computed only over ACL-authorized snapshot candidates. | A different provider/runtime registry preserves the same all-or-nothing composition, authorization, and provenance contract. |
| D-054 | Execute and report the five stored attack payloads through deterministic forbidden-effect controls while keeping the 200-cell matrix at 0 full Replay executions. | All five payloads exercise their expected control effects, but repeating those payloads across identifier combinations would not create twenty genuine variants or two hundred Replay results. | Distinct reviewed variants and checksummed full Replay outcomes replace the bounded detector evidence. |
| D-055 | Execute the declared 200-cell matrix exactly as contracted through guarded-v1's recorded-fixture production path and retain every per-cell outcome, including failures. | This closes D-051's execution-evidence gap without rewriting the held-out candidate data or fabricating payload diversity. Each cell has manifest, materialization, run, audit-chain, final-event, and result hashes. The 2026-08-12 run produced 200 passes and 0 failures. The public scorecard derives counts from those 200 rows. | A new reviewed matrix version replaces the single shared hostile artifact with distinct family/variant payloads and distinct semantic truths; any later run is a new immutable execution artifact. |
| D-056 | Disable `setup-node` pnpm caching while CI bootstrap uses the isolated `/tmp/resolveflow-pnpm-store`. | All three push jobs completed their main checks but failed during cache cleanup because setup-node tried to save its default pnpm store, which bootstrap never created. Cache is an optimization and must not turn successful validation into a failure. | CI moves bootstrap to pnpm's discovered default store or explicitly configures one shared store before setup-node cache discovery. |
| D-057 | Keep the supplied Cohere evaluation credential only in the ignored local `.env`; load it into API/worker containers, but keep public live mode and every Slack/Jira/action-write switch disabled. | A bounded 2026-07-26 smoke authenticated Command A+, returned 1,024-dimensional Embed v4 vectors, and ranked the relevant document first with Rerank v4 Fast. Live probing also found that Cohere strict schemas reject Pydantic string constraints and reports token counts as floats; the adapter now projects schemas onto the documented supported subset and records the SDK's total token counts. Billing ownership was not independently verified, so no spend claim is made. | The credential is rotated/revoked, Cohere changes its API contract, or a tested private executor replaces the local-only profile. GitHub Pages remains credential-free. |
| D-058 | A request for a public live deployment was received. The supplied provider credential is treated as secret material and is neither recorded nor committed. The public site remains static until a verified, no-billing backend host is already authenticated and the provider's quota/billing state is evidenced. | GitHub Pages cannot host the FastAPI/worker runtime; the repository has no configured public backend host. The project stop condition prohibits creating paid infrastructure or making unverified chargeable provider calls. | An already-authenticated zero-cost host with no billing requirement is available, provider quota/billing evidence is recorded, and the bounded executor passes its live deployment checks. |
| D-059 | Present the public website as a static, credential-free technical case study with an explicit public/local/runtime boundary, dated verification figures, and direct evidence links. | Recruiters should be able to distinguish the GitHub Pages snapshot from the locally implemented FastAPI/worker/PostgreSQL system while still seeing the breadth of the engineering work. All shown figures retain their recorded scope and date. | New independently verified evidence justifies replacing a dated figure or expanding the public runtime. |
| D-060 | Publish one explicitly authorized local Cohere hero result as a sanitized static `live_provider` snapshot beside, not instead of, the deterministic recorded fixture. Keep public interactive inference disabled and retain synthetic-data, technical-preview, and NO SHIP limits. | The 2026-08-12 run exercised the shared Chat/Embed/Rerank composition, retained provider/model/usage/hash evidence, passed exact citation and audit-chain verification, stopped at `needs_review` for the missing cluster ID, and performed no external write. The task authorized provider use and static publication; no spend amount or live-quality conclusion is inferred. | The credential is revoked, the artifact fails integrity checks, or stronger independently reviewed live evidence supersedes this bounded run. |

## 14. Side-by-side demo and structured-output stress decisions

| ID | Decision / fact | Rationale / evidence | Reversal trigger |
|---|---|---|---|
| D-061 | Keep the one-URL public demo recorded-only and deep-link it at `#side-by-side`. | GitHub Pages is a static export and has no server-side credential boundary. The paired fixture traces already use the shared Resolve/Replay path and visibly prove that the restricted Refund processing runbook enters unsafe retrieval but is refused before guarded retrieval. | A tested public executor gains a server-side secret boundary without paid infrastructure or public write authority. |
| D-062 | Retain the first structured-output execution as voided evidence and rerun once using the repository's existing live structured-pass output allowance. | The first execution saturated its output allowance across the clean and injected conditions, confounding schema behavior with truncation; the same allowance also made every attempted repair fail. The retained run removes that confound without changing the requested conditions, model, temperature, or seed. Both raw artifacts remain published. | A reproduction disproves the classification or the provider contract changes and a newly versioned study is required. |
| D-063 | Report schema repair as unmeasured for the retained run and distinguish it from the transport retry that actually recovered. | Every retained initial response passed JSON/schema validation, so no application repair call occurred. A rate-limit response did occur and the counted transport retry succeeded; treating it as retry-to-valid would conflate transport recovery with malformed-output recovery. | A future retained response is malformed and exercises the bounded repair path with recorded latency and token cost. |

## 15. Completion-budget follow-up decisions

| ID | Decision / fact | Rationale / evidence | Reversal trigger |
|---|---|---|---|
| D-064 | Set the evaluation budget to eight provider calls, five tool rounds, and one explicit render reserve; do not raise it again after the bounded live run failed the completion target. | Fixture stage instrumentation measured three calls and one tool round per run. Historical live traces measured a median of six calls to render and observed a fifth tool round. The new live run had no tool-round-exhausted terminals; rate limits, token limits, and semantic reference failures became binding. | A new versioned measurement with raw call-stage evidence identifies a different bounded requirement. |
| D-065 | Treat the six live `structured_response_invalid` results as schema-valid semantic-reference failures and tighten field-specific render allowlists without adding a broader retry. | Every raw output parsed and satisfied field types. Five guarded outputs referenced unsupported claims, and four outputs across both builds used a non-route claim as a route. The deterministic validator correctly rejected them. | A live rerun after provider capacity recovers supplies new evidence about the tightened contract. |
| D-066 | Publish the reduced live confirmation as an unfavorable, non-comparable follow-up rather than claiming a completion-rate improvement. | The projected full scenario set exceeded the recorded planning ceiling, so eleven scenarios were selected without reducing budgets. Guarded completed zero of eleven and unsafe completed four of eleven; persistent rate limiting prevented a post-prompt live validation. | A full-scope, versioned live run under the same declared budgets completes without the observed capacity constraint. |

| D-067 | Fail public-live admission closed until a bounded executor can persist, execute, and complete tickets. | The limiter could previously return `202 queued` but no runtime consumed or completed the ticket. Accepting work that cannot run is less safe than returning the complete recorded fallback with `public_live_executor_unavailable`. | A tested executor owns durable ticket state, deadlines, completion, failure fallback, and configured live composition without external writes. |

## 16. Cohere integration hardening decisions

| ID | Decision / fact | Rationale / evidence | Reversal trigger |
|---|---|---|---|
| D-068 | Treat the broad post-completion improvement request as authority for reversible repository hardening and local verification only. Do not infer authority for a new paid provider run, connector write, deployment, milestone expansion, or relabeling of retained evidence. | The repository already contains bounded live Cohere artifacts and explicit stop conditions. This pass can strengthen contracts, tests, and presentation without manufacturing a newer model result or changing external state. | The user explicitly authorizes a separately budgeted live run, connector exercise, or deployment and its prerequisites pass. |
| D-069 | Map the Cohere V2 contract explicitly: strict tool schemas, uppercase tool-choice/citation options, content-indexed native citation spans, a last evidence slot that can force `tool_choice=NONE`, and a tool-free final structure pass whose JSON Schema is bound to the exact verified graph. Keep local semantic validation and safe provider-error normalization as independent fail-closed boundaries. | The provider can constrain syntax and expose native citations, while ordinary code must still validate source blocks, graph references, allowed field kinds, unknown/conflict preservation, and deterministic rendering. | A versioned Cohere API/SDK contract replaces these shapes and equivalent local checks pass against recorded and live evidence. |
| D-070 | Bind tool-result evidence to canonical data, operation, status, timestamp, checksum, and unique nonblank provenance; cap calls per round/run and each tool timeout by the remaining wall clock. Hostile or instruction-like tool output cannot independently support a claim, and only verified `ACTION` claims may be action-supporting. | Tool output is untrusted evidence, not authority. Defense in depth is required in tool execution, verification, finding validation, and action proposal creation. | A stronger typed evidence transport and policy engine preserves or improves the same integrity, time, authorization, and effect boundaries. |
| D-071 | Validate Rerank requests and responses at the adapter boundary: the requested model must match the active adapter, Pro calls require an escalation reason, `max_tokens_per_doc` is explicit, `top_n` is feasible, response cardinality is exact, indexes are unique/in range, and scores are finite. Retain Cohere SDK 7.0.5 until an upgrade is separately compatibility-tested. | Silent model substitution, truncated response sets, or non-finite scores corrupt downstream ranking evidence. The installed SDK already supports the required documented V2 shapes, so an untested dependency upgrade would add risk without evidentiary value. | A tested SDK/model-policy upgrade passes the contract, retrieval, typing, and credential-free suites and produces a new dated compatibility record. |
| D-072 | Add an artifact-derived `/cohere` integration receipt that separates Cohere model capabilities from ResolveFlow authority controls and labels retained live evidence, offline post-run fixes, void quality metrics, and zero writes distinctly. | A reviewer should be able to see how Command A+, Embed v4, Rerank v4, tools, citations, and structured output compose without mistaking a static case study for a public inference endpoint or a local fix for a newer live result. | A newer immutable run or a reviewed product narrative supersedes this page without weakening evidence boundaries. |
| D-073 | Regenerate only derived Cohere A/B publication prose and static projections offline, preserving the retained execution commit and all raw trial metrics. Deduplicate repeated variant IDs only in human-readable lists. | The prior projection contained stale completion wording and displayed repeated trial IDs as though they were distinct variants. Correcting derived copy must not imply another provider call or rewrite retained measurements. | A newly executed immutable A/B supersedes the retained artifact and is published with its own commit, ledger, sidecars, and validity assessment. |
| D-074 | Treat the wall-clock allowance as one propagated deadline, not a fresh timeout per retry or tool. Every production chat provider implements a bounded call; Cohere uses its native request timeout with SDK retries disabled; the counted evaluation wrapper caps throttle, backoff, retries, and each attempt against the same absolute deadline. Fixed tool handlers are cooperative async operations whose cancellation completes before a timeout result returns, including when a synchronous Resolve call originates on an active event-loop thread. | Detached worker cancellation cannot stop an in-flight HTTP call or already-running tool and can leak work after a run reports timeout. One deadline plus joined/cooperative execution keeps accounting and returned state aligned. A remote service may still finish server-side processing after the client disconnects, so the attempt remains counted. | A provider-native end-to-end cancellation token or a terminable isolation boundary demonstrably preserves the same no-detached-work, call-accounting, and wall-budget guarantees. |
| D-075 | Publish only the single complete 32-cell live cohort whose snapshots share one execution timestamp and identity. Quarantine 63 snapshots from other invocations, recover metrics offline with zero provider calls, preserve the historical budget schema verbatim, regenerate corpus evidence from frozen manifests, and reconcile the 233-call ledger as 27 dry-pass plus 206 selected-full-pass calls. Record `uncommitted` as the execution git state and the current commit only as a separate publication base. | Combining complete cells from different timestamps, policies, or invocation attempts creates a synthetic aggregate that was never executed as one declared experiment. Fail-closed cell, hash, frozen-input, corpus, and ledger checks now make that mismatch mechanically visible while preserving all excluded evidence for audit. | A newer pre-registered, immutable live run publishes a complete cohort, exact execution commit/diff, and its own reconciled ledger without reusing these snapshots. |
| D-076 | Supersede only the hard-ceiling/cancellation-guarantee reading of D-025/D-074 while retaining one propagated remaining-time allowance. Provider-call, agent-round, and tool-call limits are hard pre-dispatch caps. `max_total_tokens` uses preflight sizing and post-response observed usage to decide whether another Chat call may start; one in-flight response can cross it, so it is not a hard token or billing ceiling. Remaining time is supplied to deadline-aware operations and checked after return, making the timeout cooperative and per operation rather than a guaranteed hard total deadline. Chat accounting uses one complete token pair or one complete billed-token pair without mixing units; Embed input tokens and Rerank search units are recorded only when Cohere returns them, and historical ledgers are not backfilled. | Exact usage is unavailable until a response arrives, client cancellation cannot prove remote work stopped, and retained evidence must not manufacture absent usage units. Count caps still prevent new calls deterministically, while deadline/usage overruns terminate subsequent work visibly and the attempted call remains counted. | A versioned provider contract supplies and verifies server-side cumulative token enforcement plus end-to-end cancellable deadlines for every dependency; new evidence may then support a stronger hard-ceiling claim without relabeling historical runs. |
