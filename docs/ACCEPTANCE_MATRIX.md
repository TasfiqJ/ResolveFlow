...
| X-15 | Push validation and Pages do not fail while saving an unused pnpm cache path | workflow audit plus successful `validate` run `30208242771` and Pages run `30208242773` | post-release CI repair | PASS |
| X-16 | Local Cohere composition authenticates Chat, Embed, and Rerank while retaining deterministic verification and disabled external writes | bounded 2026-07-26 adapter smoke plus one local hero run; 6 provider calls, 16,002 observed tokens, 3/3 claims unsupported, `needs_review`, no action, 0 successful forbidden effects | post-release local provider setup | PASS (integration/fail-closed behavior only; no live quality claim) |
| X-17 | Static public case study explicitly separates GitHub Pages output from local API/worker/Cohere capabilities, presents dated verification evidence, and retains all release limits | `pnpm --dir apps/web test -- page.test.tsx && pnpm --dir apps/web typecheck && pnpm --dir apps/web lint && NEXT_PUBLIC_BASE_PATH=/ResolveFlow pnpm --dir apps/web build && NEXT_PUBLIC_BASE_PATH=/ResolveFlow pnpm --dir apps/web e2e` | post-release transparency | PASS (local; 2 component and 9 Chromium/Axe tests) |
